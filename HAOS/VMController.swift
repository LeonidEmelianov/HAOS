...
final class VMController: NSObject, VZVirtualMachineDelegate {
    ...
    /// ~/Library/Application Support/HAOSMenuBar/
    private let stateDirectory: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory,
                                            in: .userDomainMask)[0]
        let dir = base.appendingPathComponent("HAOSMenuBar", isDirectory: true)
    ...
    private func createConfiguration() throws -> VZVirtualMachineConfiguration {
        ...
        guard FileManager.default.fileExists(atPath: diskURL.path) else {
            throw NSError(domain: "HAOSMenuBar", code: 1, userInfo: [
